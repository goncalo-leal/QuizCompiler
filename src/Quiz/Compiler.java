...
public class Compiler extends quizBaseVisitor<ST> {
   ...
   @Override public ST visitCreateQuiz(quizParser.CreateQuizContext ctx) {
      ...
      ST res = null;
      String quizname = ctx.STRING(0).getText();
      if (ctx.STRING(1) != null) {
         res = stg.getInstanceOf("createQuizPassword");
         String pass = ctx.STRING(1).getText();
         res.add("quizname", quizname); 
         res.add("pass",pass);
         return res;
      }
      else {
         res = stg.getInstanceOf("createQuiz");
         res.add("quizname", quizname); 
         return res;
      }
   }
   ...
   @Override public ST visitCreateQuizCorrection(quizParser.CreateQuizCorrectionContext ctx) {
      ...
      String quizname = ctx.STRING(0).getText();
      if (ctx.STRING(1) != null) {
         res = stg.getInstanceOf("createQuizAutoCorrectivePassword");
         String password = ctx.STRING(1).getText();
         res.add("quizname", quizname); 
         res.add("number", ctx.NUMBER());
         res.add("pass",password);
         return res;
      }
      else {
         System.out.println("entrou no else");
         res = stg.getInstanceOf("createQuizAutoCorrective");
         res.add("quizname", quizname); 
         res.add("number", ctx.NUMBER());
         return res;
      }
   ...
   @Override public ST visitMultichoice(quizParser.MultichoiceContext ctx) {
      ...
      String id = ctx.NAME(0).getText();
      String title = "";
      if (ctx.title() != null) {
         title = ctx.title().STRING().getText();

      } else if (ctx.NAME(1) != null) {
         title = ctx.NAME(1).getText().replaceAll("\"","");
      }
   ...
   @Override public ST visitMatching(quizParser.MatchingContext ctx) {
      ...
      if (ctx.title() != null) {
         text = ctx.title().getText();

      } else if (ctx.NAME(1) != null) {
         text = ctx.NAME(1).getText().replaceAll("\"","");
      }
   ...
   @Override public ST visitGetAnswerToChoice(quizParser.GetAnswerToChoiceContext ctx) {
      ST res = stg.getInstanceOf("getAnswerToChoicee");
      return res;
   }
...
String text = "";
   ...
   // done
   @Override public ST visitNumberVariable(quizParser.NumberVariableContext ctx) {
      ST res = stg.getInstanceOf("numberVariable");

      String var = ctx.NAME().getText();
      String value = ctx.mark().getText();
   ...
   // done
   @Override public ST visitTextVariable(quizParser.TextVariableContext ctx) {
      ST res = stg.getInstanceOf("textVariable");
