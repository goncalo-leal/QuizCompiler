...
public class QuizSemantic extends quizBaseVisitor<Boolean> {
   ...
   @Override public Boolean visitNumberVariable(quizParser.NumberVariableContext ctx) {
      verifyShow(false);
      verifyChoice(false);
      if (!quizCreated()){
         return false;
      ...
      if (variaveis.keySet().contains(ctx.NAME().getText())) {
         System.err.println("Variable " + ctx.NAME().getText() + " already exists.");
         return false;
      }
      else {
         variaveis.put(ctx.NAME().getText(), "number");
      }

      return true; // number ou float
   }

   @Override public Boolean visitTextVariable(quizParser.TextVariableContext ctx) {
      verifyShow(false);
      verifyChoice(false);
      if (!quizCreated()){
         return false;
      ...
      if (variaveis.keySet().contains(ctx.NAME().getText())) {
         ...
         variaveis.put(ctx.NAME().getText(), "text");
      }
   ...
   @Override public Boolean visitMultichoice(quizParser.MultichoiceContext ctx) {
      ...
      if (questions_ids.keySet().contains(ctx.NAME(0).getText())) {
         System.err.println("Já existe uma pergunta com o ID " + ctx.NAME(0).getText());
         return false;
      }

      if (ctx.NAME(1) != null && !variaveis.keySet().contains(ctx.NAME(1).getText())) {
         System.err.println("A variável " + ctx.NAME(1).getText() + " não está definida.");
         return false;
      }
      ...
      if(ctx.multichoiceoption() != null){
         ...
         while(iterador.hasNext()){
            ...
            if (options.contains(opt_id)) {
               System.err.println("A pergunta com o ID " + ctx.NAME(0).getText() + " já contem uma opção com o ID " + opt_id);
               return false;
            }
            options.add(opt_id);
      ...
      questions_ids.put(ctx.NAME(0).getText(), "MultiChoice");
      questions_options.put(ctx.NAME(0).getText(), options);    
   ...
   @Override public Boolean visitMatching(quizParser.MatchingContext ctx) {
      ...
      questions_ids.put(ctx.NAME(0).getText(), "Matching");
      questions_options.put(ctx.NAME(0).getText(), options);  
      questions_answers.put(ctx.NAME(0).getText(), new ArrayList<String>());
   ...
   @Override public Boolean visitImport_expr(quizParser.Import_exprContext ctx) {
      ...
      for (quizParser.PContext p : ctx.plist().p()) {
         ...
         if (bancosP.get(ctx.NAME().getText()).get(p.NAME(0).getText()).getType().equals("MultiChoice") || bancosP.get(ctx.NAME().getText()).get(p.NAME(0).getText()).getType().equals("Matching")){
   ...
   @Override public Boolean visitRespostaCurta(quizParser.RespostaCurtaContext ctx) {
      ...
      if (questions_ids.containsKey(ctx.NAME(0).getText())) {
         System.err.println("Já existe uma pergunta com o ID " + ctx.NAME(0).getText());
         return false;
      }

      if (ctx.STRING() == null && checkCreateQuizAutocorrective) {
         System.err.println("O quiz é de auto correção, logo deve definir uma resposta à pergunta com ID " + ctx.NAME(0).getText());
         return false;
      }
      ...
      questions_ids.put(ctx.NAME(0).getText(), "short");
      questions_answers.put(ctx.NAME(0).getText(), answers);
   ...
   @Override public Boolean visitNumericQuestion(quizParser.NumericQuestionContext ctx) {
      ...
      questions_ids.put(ctx.NAME(0).getText(), "numeric");
      questions_answers.put(ctx.NAME(0).getText(), answers);
   ...
   @Override public Boolean visitEssayQuestion(quizParser.EssayQuestionContext ctx) {
      ...
      questions_ids.put(ctx.NAME(0).getText(), "essay");
   ...
   @Override public Boolean visitTitle(quizParser.TitleContext ctx) {
      ...
      return true; // ou é String ou não é
   }
